...
def currentIndexFutureList():
    ...
    sec_IH["expiredate"] = windDataTransform(w.wsd(s, "lasttrade_date"))[
        "LASTTRADE_DATE"
    ].apply(lambda x: str(x)[:10])
    today = datetime.datetime.now().strftime("%Y-%m-%d")
    sec_IH["expiredate"] = sec_IH["expiredate"].apply(
        lambda x: trading_day.index(x) - trading_day.index(str(today))
    )  # 用交易日重新定义还剩下的天数
    return sec_IH.loc[:, ["wind_code", "sec_name", "expiredate"]]


def currentOptionList():
    sec_call = windDataTransform(
        w.wset("OptionChain", "us_code=510050.SH;call_put=call")
    ).loc[:, ["option_code", "option_name", "strike_price", "month", "expiredate"]]
    sec_call = sec_call.assign(call_put="call")
    sec_put = windDataTransform(
        w.wset("OptionChain", "us_code=510050.SH;call_put=put")
    ).loc[:, ["option_code", "option_name", "strike_price", "month", "expiredate"]]
    sec_put = sec_put.assign(call_put="put")
    r = pd.concat([sec_call, sec_put])
    today = datetime.date.today()
    r["expiredate"] = r["expiredate"].apply(
        lambda x: trading_day.index(str(today + datetime.timedelta(x)))
        - trading_day.index(str(today))
    )
    return r
...
def optionRealtime(optionList):
    ...
    option_real = windDataTransform(
        w.wsq(
            ",".join(l),
            "rt_pre_settle,rt_last,rt_ask1,rt_asize1,rt_bid1,rt_bsize1,rt_ask2,rt_asize2,rt_bid2,rt_bsize2,rt_ask3,rt_asize3,rt_bid3,rt_bsize3",
        )
    )
...
def indexFutureRealtime(ifList):
    ...
    index_future = windDataTransform(
        w.wsq(",".join(l), "rt_pre_settle,rt_last,rt_ask1,rt_asize1,rt_bid1,rt_bsize1")
    )
...
r = r.loc[
    :, ["option_code", "option_name", "strike_price", "month", "expiredate", "call_put"]
]
